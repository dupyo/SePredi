...
import poly.util.KakaoDisConnectToken;
import poly.util.KakaoLogout;
...
public class UserController {
	...
	public String kakaoLogin(@RequestParam("code") String code, RedirectAttributes ra, HttpServletRequest request, HttpSession session, HttpServletResponse response, Model model) throws IOException {
		...
		if (id.isEmpty()) {
			...
			model.addAttribute("url", url="https://kauth.kakao.com/oauth/authorize?client_id="+clientID+"&redirect_uri=https://localhost:8443/kakaologin.do&response_type=code");
		} else {
	...
	public String kakaoLogout(HttpServletRequest request, HttpServletResponse response, Model model, HttpSession session) throws Exception {
		...
		JsonNode node2 = KakaoLogout.kakaoLogout(accessToken);
		//결과 값 출력
		System.out.println("로그아웃 후 반환되는 아이디 : " + node2.get("id"));

		/*log.info("accessToken : " + accessToken);
		JsonNode node1 = KakaoDisConnectToken.disconnectToken(accessToken);
		//결과 값 출력
	...
	public String kakaoLogout(@RequestParam("code") String code, HttpServletRequest request, HttpServletResponse response, Model model, HttpSession session) throws Exception {
		...
		log.info(code);
		JsonNode node = KakaoDisConnectToken.Logout(code);
		//결과 값 출력
